...
public class SaveCD extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
   	...
   	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
   		...
   		if(cd_type.equals("international")) {
   	...
   	}else if(cd_type.equals("special")) {
   		String feature=request.getParameter("newfeature");
		Session session1=factory.openSession();
   	    session1.beginTransaction();
   	   // session1.persist(data);
   	    session1.persist(new SpecialEditionCD(data.getTitle(),data.getArtist(),data.getPurchasedate(),data.getCost(), feature));
   		session1.getTransaction().commit();
